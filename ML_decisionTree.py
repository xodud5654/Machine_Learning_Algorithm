...
for depth in np.arange(1,5):
    dt2 = DecisionTreeClassifier(criterion = 'gini',
                                max_depth = depth,
                                random_state=42)
    dt2.fit(X_tr_std, y_tr)
    y_val_pred = dt2.predict(X_val_std)
...
print("best depth values: %.3f" %depth_val[idx])

dt_opt = DecisionTreeClassifier(criterion = 'gini',
                                max_depth = depth_val[idx],
                                random_state=42)
dt_opt.fit(X_train_std, y_train)    
y_test_pred = dt_opt.predict(X_test_std)
...
y_test_prob = dt_opt.predict_proba(X_test_std)
